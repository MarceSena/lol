...
class TokensAutorizadosRepository
{
    ...
    public function validarToken($token)
    {
        ...
        if ($token) {
            ...
            $stmt->bindValue(':status', ConstantsUtil::SIM);
            $stmt->execute();
            if ($stmt->rowCount() !== 1) {
                header("HTTP/1.1 401 Unauthorized");
                throw new InvalidArgumentException(ConstantsUtil::MSG_ERRO_TOKEN_NAO_AUTORIZADO);
            }
        } else {
            throw new InvalidArgumentException(ConstantsUtil::MSG_ERRO_TOKEN_VAZIO);
        }
    }
